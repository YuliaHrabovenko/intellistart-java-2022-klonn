...
import static com.intellias.intellistart.interviewplanning.exceptions.ExceptionMessage.INTERVIEWER_SLOT_BOUNDARIES_EXCEEDED;
...
public final class PeriodUtil {
  ...
  public static void validatePeriod(LocalTime from, LocalTime to) {
    if (from.isBefore(minStartTime) || to.isAfter(maxEndTime)) {
      throw new ValidationException(INTERVIEWER_SLOT_BOUNDARIES_EXCEEDED.getMessage());
    }
